...
struct HomeView: NavigatableView {
    func configure(view: NavigationConfig) -> NavigationConfig { view.ignoresSafeArea(.bottom) }
    var body: some View {
        VStack(spacing: 0) {
            createNavigationBar()
...
private extension ActionsView {
    func createItem(_ index: Int) -> some View { Button(action: { onItemTap(index) }) {
        VStack(spacing: 0) {
...
private extension ActionsView {
    func onItemTap(_ index: Int) { switch index {
        case 0: SendMoneyView().push(with: .verticalSlide)
        case 1: return
        default: fatalError()
    }}
}
